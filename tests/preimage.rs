...
fn test_preimage_wast() {
...

    {
        ...
        assert_eq!(
            ...
            Some(Value::Str {
                hint: String::new(),
                data: Arc::from("for great justice, move every zig!")
            })
        );
...
fn test_preimage_wasm() {
    // Skip test if WASM files not built (requires wat2wasm tool)
    let Some(unlock_script) = load_wasm("preimage_unlock.wasm") else {
        eprintln!("Skipping test_preimage_wasm: preimage_unlock.wasm not found (run 'make' in examples/wast to build)");
        return;
    };
    let Some(lock_script) = load_wasm("preimage_lock.wasm") else {
        eprintln!("Skipping test_preimage_wasm: preimage_lock.wasm not found (run 'make' in examples/wast to build)");
        return;
    };
